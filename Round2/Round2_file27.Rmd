---
title: "Arbitrary Covariance from eigenvalues. Signal in HIGH PCs of noise"
...
# ei.vals0 <- seq(1:p)
# ei.vals <- ei.vals0/sum(ei.vals0)*length(ei.vals0)
# plot(ei.vals, type='h')
# A <- matrix(rnorm(p^2),nrow=p)
# V <- eigen(t(A)%*%A)$vectors
# D <- diag(ei.vals)
# Sigma <- V%*%D%*%t(V)
# save(Sigma, D, V, file='Sigma27.RData')

load(file='Sigma27.RData')
