...
Name & Algorithm & Accuracy & Parameters\\ 
\hline
\hline
Hotelling & Hotelling & -- & -- \\ 
...
Another example is \textit{lda.noCV.1}, which is Fisher's LDA, with a substituted accuracy estimate.}
\label{tab:collected}
\end{tcolorbox}
...
Theory suggests that all permutation tests should be exact, i.e., exactly control the false positive rate. 
Our simulations confirm this: in Figure~\ref{fig:simulation_1}, and all the following, a red circle encodes the null case,  where no signal is present and $x|y=1$ has the same distribution as $x|y=-1$.
We see that the permutation test is exact for all candidate test statistics. In particular if: \newline
