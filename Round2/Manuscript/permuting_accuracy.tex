...
Via simulation, we compare various test statistics: some based on classifier and some based on multivariate tests. 
...
For the cases that the purposes of the analysis is not signal detection, but rather to evaluate the performance of a particular classifier, we suggest several improvements to increase power. In particular, to replace V-fold cross validation with the leave-one-out Bootstrap.
\end{abstract}
...
It demonstrates than the the power disadvantage of accuracy tests, cannot be remedied by random tie breaking.
