...
The simulations results reveal some counter intuitive phenomena. 
First, the \emph{Oracle} is not the best performer: the location tests with estimated covariance have more power than a location test with the generative covariance. 
This may be due to the fact that the permutation covariance differ from the sampling covariance. 
A difference than grows larger as the dimension of the problem increases. 

Second, the power of some tests may be smaller than then false positive rate. 
Particularly when the signal is in the direction of the lower principal components (PCs) of $\Sigma$. 
This is known as a \emph{biased test} in the statistical literature, and has already been observed in the context of high-dimensional testing by \citet{goeman2006testing}, albeit not in the context of permutation tests.

It may be possible that even for shift alternatives, an accuracy test has more power than a location test. 
...
Phenomenon:
Oracle worse than other locations tests.
Location tests biased when noise in low PCs.
Low PCs seems harder than high PCs.

[TODO: performs more replications and update discussion.]


\begin{figure}[h]
	\centering
	\caption{Short memory, AR(1) covariance. $\Sigma_{k,l}=\rho^{|k-l|}; \rho=0.6$}	
...
\begin{figure}[h]
	\centering
	\caption{Long-memory Brownian motion covariance: $\Sigma_{k,l}=\rho \min\{k,l\}; \rho=0.2$.}	
	\begin{subfigure}[t]{.3\textwidth}
		\centering
...
\begin{figure}[h]
	\centering
	\caption{Arbitrary Covariance. $\Sigma:=A'A,$ where $A$ is a Gaussian $p\times p$ random matrix with independent $\mathcal{N}(0,1)$ entries.}	
	\begin{subfigure}[t]{.3\textwidth}
		\centering
...
\label{sec:scale_alternative}
Locations tests, as their name suggests, are designed to detect changes in multivariate location.
Are they also preferable to detect changes in scale, where $\mathbb{E}[x|y=1]=\mathbb{E}[x|y=0]$, but $Var[x|y=1]\neq Var[x|y=0]$?
There is no universal answer to this question: the preferred test depends on the alternative/signal at hand. 

In Figure~\ref{fig:scale_change} we report a simulation designed to demonstrate a preference for accuracy tests. 
We could have just as easily designed a simulation to demonstrate a preference for location tests. 
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.5\linewidth]{"art/file8"}
	\caption{TODO: what is wrong here?!?
		\textbf{Scale Change.} 
...
\begin{tcolorbox}
	...
	\begin{tabular}{l|c|c|c|c}
		...
		SVM.Boot.2 & SVM & bLOO 	& $50$ & cost=10 \\ 
	\end{tabular} 
	\captionsetup{type=table}
	\caption{
...
\begin{figure}[ht]
	...
	\caption{
		TODO: redo whole Bootstrap analysis.
		\textbf{Bootstrap.}
...
\begin{tcolorbox}
	...
	\begin{tabular}{l|c|c|c}
		...
		LDA.highdim.4 & LDA & b$0.632$ [TODO: replace with LOO] & B=50 \\ 
	\end{tabular} 
	\captionsetup{type=table}
	\caption{
