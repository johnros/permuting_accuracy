...
This was not apparent when the signal was in the high PC, because it had a large Euclidean norm. 
When the signal is in the low PC, and $\mu'\Sigma^{-1}\mu$ is fixed, then $\mu$ has a small Euclidean norm. 
...
\begin{figure}[h]
		...
		\begin{subfigure}[t]{.4\textwidth}
			\centering
	\includegraphics[width=0.5\linewidth]{"art/file26"}
	\caption{TODO: figure.}  
...
TODO: add n=400 to isolate NPL versus discretization with 1,000 replications
...
Our choice implies that the Euclidean norm of $\mu$ varies with the covariance, and with the direction of the signal, so that detecting signal in the low PCs is not easier than detecting signal in the high PCs.
Quantitatively, fixing the Mahalanobis norm implies that the Euclidean norm of $\mu$ is proportional to the eigenvalue of the corresponding eigenvector: $\mu$ will have a large Eucliden norm in the high PCs and small Eucliden norm in the low PCs.

[TODO: add simulation with fixed Eucliden norm]



\subsection{High Versus Low PCs}
TODO. In particular bias in low PCs.
