...
By this we mean that both \emph{SD} and \emph{Hotelling.shrink} alleviate the dimensionality of the problem, manifested in the variability in the estimation of $\Sigma$, by introducing some bias. 

At first, this may seem as a false intuition, because we know from ridge regression that $l_2$ regularization, such as in our SVM, already introduces a Tikhonov regularization to the covariance estimation.  
This, however, is not the only way to regularize the covariance estimator. 
...
See, for instance, \emph{SVM.CV.6} is clearly better regularized than other the V-fold cross-validated accuracy tests. 
Compounding with a bootstrap instead of V-fold further increases the power of the test (\emph{LDA.highdim.4}).
