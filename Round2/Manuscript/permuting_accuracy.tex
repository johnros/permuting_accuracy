...
For the cases in which the purpose of the analysis is not signal detection, but rather to evaluate the performance of a particular classifier, we suggest several improvements to increase power. 
In particular, to replace V-fold CV with the leave-one-out Bootstrap.
\end{abstract}
...
Many neuroscientists and geneticists detect signal by fitting a classifier and testing whether it's prediction accuracy is better than chance. 
The workflow in consists of fitting a classifier, and estimating its predictive accuracy using cross validation. 
...
This is true, but the multivariate-statistics literature provides many multivariate effect-size estimators, most directly generalize Cohen's d, and not suffer from discretization like accuracy estimates. 
