...
To keep the comparisons ``fair'' as the correlations vary, we kept the Mahalanobis norm of the signal fixed. 
The question remains- which test has more power: accuracy or location?
We address this question using various correlation structures.
We also vary the direction of the signal, $\mu$, and distinguish between signal in the direction of the highest principal component (PC) of $\Sigma$, or the lowest. 

The simulation results reveal some non trivial phenomena.
First, when the signal is in the direction of the highest PC, the high dimensional location tests are far superior than accuracy tests. 
This holds true for the various correlation structures: the short memory correlations of $AR(1)$ in Figure~\ref{fig:dependence_11}, the long memory correlations of a Brownian motion in Figure~\ref{fig:dependence_21}, and the arbitrary correlation in Figure~\ref{fig:dependence_31}.

When the signal is in the direction of the lowest PC, a different phenomenon appears.
There is no clear preference between location or accuracy tests.
Instead the non-regularized tests are the clear victors. 
This holds true for the various correlation structures: the short memory correlations of $AR(1)$ in Figure~\ref{fig:dependence_12}, the long memory correlations of a Brownian motion in Figure~\ref{fig:dependence_22}, and the arbitrary correlation in Figure~\ref{fig:dependence_32}.
We attribute this phenomenon to the bias introduced by the regularization, which masks the signal.
This was not apparent when the signal was in the low variance PC, because it had a large Euclidean norm. 
When the signal is in the high variance PC, and $\mu'\Sigma^{-1}\mu$ is fixed, then $\mu$ has a small Euclidean norm. 
The reader may argue that we should not have fixed $\mu'\Sigma^{-1}\mu$, thus varying $\mu$ with $\Sigma$, but rather fix the Euclidean norm of $\mu$. 
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.4\textwidth}
		...
		\caption{$\mu$ in the highest PC of $\Sigma$.}  
		\label{fig:heteroskedastic_11}	
	\end{subfigure}
	\begin{subfigure}[t]{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{"art/file26"}
		\caption{TODO: update graphics. $\mu$ in the lowest PC of $\Sigma$.}  
...
This type of analysis render resubstitution accuracy estimates a legitimate asymptotic test, while they are a poor finite sample test. \newline
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{art/file10}
	\caption{[TODO: more replications] $n=400$}
	\label{fig:file10}
\end{figure}
...
\label{sec:fix_snr}
For a fair comparisons between simulations, in particular between those with different correlations, we needed to fix the difficulty of the problem.
...
Formally, in all our simulations we set $\mu \Sigma^{-1} \mu=p$.
...
For completeness, Figure~\ref{fig:dependence_4} reports the power analysis under $AR(1)$ correlations, but with $\Vert \mu \Vert_2$ fixed in stead of $\Vert \mu \Vert_\Sigma$.
...
Our intuition is confirmed in that all statistics find it easier to detect signal in smaller variance PCs.
It is also consistent with Figure~\ref{fig:dependence_1}, in that 
...
\subsection{High Versus Low PCs}
Figures \ref{fig:dependence_1}, \ref{fig:dependence_2}, \ref{fig:dependence_3} and \ref{fig:dependence_4}, demonstrate that detecting signal in the direction of the low PCs (i.e. high-variance PCs), is very different than detecting in the high PCs.
Even when fixing $\Vert \mu \Vert_\Sigma$, so that the difficulty of the problem is fixed, we see power differences? 
Why is that?
...
The various regularization methods deflate the high variance directions, thus, inflate the low variance directions.
