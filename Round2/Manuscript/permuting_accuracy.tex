...
\label{eq:distribution}
	\x_i = \mu \y^*_i + \eta_i,
\end{align}
where $\y^*_i=\set{0,1}$ encodes the class of subject $i$, the noise is distributed as $\eta_i \sim \gaussp{p}{0,\Sigma}$, the sample size $n=40$, and the dimension of the data is $p=23$. 
...
We now test the robustness of our results to the correlation within sample. 
in terms of Eq.(\ref{eq:distribution}) it means we change $\Sigma$. 
Intuitively- both location tests and accuracy tests require some estimation of $\Sigma$, so that correlations are accounted for. 
The question remains- which test does it better. 
We address this matter in two setups. 
A ``short range'' dependence, for which we use an $AR(1)$ covariance, and a ``long range'' dependence, for which we use the Brownian motion covariance. 
...
\subsection{Resampling with Replacement}
\label{sec:bootstrap}
...
\centering
	  ...
	  \caption{TODO: verify!
	  	\textbf{Mixture--} $\x_i = \chi_i \mu + \eta_i; \chi_i = \set{-1,1}$ and $\prob{\chi_i=1}=(1/2-p)^{\y^*_i}  (1/2+p)^{1-\y^*_i}$. $\mu$ is a $p$-vector with $3/\sqrt{p}$ in all coordinates.
