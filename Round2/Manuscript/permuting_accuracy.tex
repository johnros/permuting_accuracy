...
We call this an \emph{accuracy test}, because it uses the prediction accuracy as a test statistic \footnote{Known as \emph{class prediction}, or \emph{pattern discrimination}.}. 
...
For brevity, and in contrast to \emph{accuracy tests}, we will call these \emph{location tests\footnote{Known as \emph{class comparisons}.}}, because they test for the equality of location of a multivariate distribution. 
...
The comparison between location and accuracy tests was precisely the goal of \cite{ramdas_classification_2016}, who compared Hotelling's $T^2$ location test to accuracy tests using \emph{Fisher's linear discriminant analysis} classifier (LDA). 
...
In our vocal/non-vocal example we have $y=\set{0,1}$ and $p$, the number of voxels in a brain region so that $x=\reals^{27}$. 

Given $n$ pairs of $(x_i,y_i)$, typically assumed i.i.d., a multivariate test amounts to testing whether $x|y=1$ has the the same distribution as $x|y=0$. 
...
An accuracy test amounts to setting a test statistic $\statistic$, to be some measure prediction accuracy.  

Denoting a dataset by $\data:=\{x_i,y_i\}_{i=1}^n$, a predictor\footnote{Known as a \emph{hypothesis} in the machine learning literature.}, $\hyp:x \to y$, is the output of a learning algorithm $\algo$ when applied to the dataset $\data$. 
...
\begin{enumerate}
\item How to estimate $\accEstim_{\algo}$? In particular, should it be resampled and how? 
\item If resampling using V-fold cross validation: 
How many folds? 
...
The standard practice when V-fold cross validating is to constrain the data folds to be balanced, i.e. stratified \citep[e.g.][]{ojala_permutation_2010}.
...
(a) Fix a test statistic $\statistic$.\newline
(b) Permute labels and recompute $\statistic$ to recover its null. \newline
...
In each replication, we generate $n$ i.i.d. samples from a shift class $\x_i = \mu \y^*_i + \eta_i$, where $y^*_i=\set{0,1}$ encodes the class of subject $i$, the noise is distributed as $\eta_i \sim \gaussp{p}{0,\Sigma}$, the sample size $n=40$, and the dimension of the data is $p=23$. 
...
Theory suggests that all permutation tests should control the false positive rates. 
...
The Neyman-Pearson type reasoning that favors the location test over accuracy tests may fail when the data is not multivariate Guassian, thus Hotelling's $T^2$ statistic no longer a generalized-likelihood-ratio test. 

To check this, we replaced the multivariate Gaussian distribution with a heavy-tailed multivariate-$t$ distribution. 
...
\begin{figure}[h]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{\textbf{Scale Change.} 
			$\x_i =  \eta_i * \mu^ {\y^*_i}$ so that the effects are a scale change.}  
...
\begin{figure}[h]
	...
	\begin{subfigure}{.4\textwidth}
		\centering
		%	  \includegraphics[width=1\linewidth]{"art/file10"}
