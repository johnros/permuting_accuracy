---
title: "Signal in HIGH PCs of noise"
...
A <- matrix(rexp(p^2,0.2), ncol = p, nrow = p)
Sigma <- crossprod(A,A)
dim(Sigma)
lattice::levelplot(Sigma)
Sigma.eigen <- eigen(Sigma, symmetric=TRUE)
plot(Sigma.eigen$values, type='h')
direction <- Sigma.eigen$vectors[,which.max(abs(Sigma.eigen$values))] 
```
...
pvals.1.9 <- foreach(j=1:n.replications, .combine=rbind) %dopar%{
  foreach(effect=effects, .combine=rbind)  %do% {
    ...
    ## Shift alternative:
    noise <- .noise
    noise[labels,] <- noise[labels,] + effect*  p * direction
