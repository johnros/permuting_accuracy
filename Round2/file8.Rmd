...
pvals.1.9 <- foreach(j=1:n.replications, .combine=rbind) %dopar%{
  foreach(effect=effects, .combine=rbind)  %do% {
    ...
    ## Gaussian noise with Scale alternative:
    noise <- matrix(NA, ncol=p, nrow=n)
    noise[!labels] <- rmvnorm(n=n/2, sigma = Sigma)
    Sigma.effect <- diag( exp(effect/4 * ((1:p)-p/2)) )
    noise[labels] <- rmvnorm(n=n/2, sigma = Sigma.effect)
    # matplot(t(noise), type='l', col=labels+1)
    ...
    # Observables 
    ts <- list(
      Oracle=t_Oracle(x1, x2, Sigma.effect, Sigma),
