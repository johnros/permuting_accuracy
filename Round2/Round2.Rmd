...
# effects <- sqrt(40/400) * seq(0, 0.5, length=3) # change in location
...
#   eigen(matrix(rnorm(1e6), ncol = 1e3, nrow = 1e3))$values[[1]][[1]]
# }
# foreach(i=1:20) %:% foreach(j=1:3) %dopar% mean(rnorm(1e7))
```
...
pvals.1.9 <- foreach(j=1:n.replications, .combine=rbind) %dopar%{
  foreach(effect=effects, .combine=rbind)  %do% {
    ...
    ## Scale alternative:
    A <- diag(n) 
    A[labels,labels] <- A[labels,labels]*(1:n)^(-effect)
    noise <- A %*% .noise 
