...
\item The presence of correlations between coordiantes reduces the signal to noise ratio (SNR), thus reduces power. 
More importantly, in the presence of correlations the effect of regularization is amplified, increasing the power difference between regulirized and non-regulirized test statistics.
...
We perform group inference using within-subject permutations alng the analysis pipeline of \cite{stelzer_statistical_2013}, which was also reported in \cite{gilron_quantifying_2016}. 
...
Using simulations, we have concluded that the population tests are typically preferable. 
...
Our empirical example suggests that even if the population test does not neccesarily domiate the accuracy test in power, empirically, it does have an advantage. 
...
We give particuar emphasis in this section to V-fold cross validation due to it popularity, but note that sampling the test set with replacement is actually preferable, as we discuss in Section~\ref{sec:bootstrap}.
...
Altough we not we did not find a power difference between balanced and unbalanced foldings. 
...
(a) the train set is larger so that better accuracies are achieveable. 
...
We attribute this to the compouding of discretization and concentrarion effects: the difference between the sampling distribution of the resubstitutution accuracy is simply indinstinguishable under the null and under the alternative. 
In low dimensional problems, the discretization is less impactful, and the computational burdon of cross validation can be avoided by using the resubstitution accuracy. 
...
\paragraph{Regulirize}
Regulirizing the accuracy test proves very useful in high dimensional problems. 
...
The discreteness of the accuracy statistic is goverend by the number of examples in the union (over all validation iterations) of test sets.
...
To study this, we call upon several regulirized classifiers, desifned for high dimensional problems. 
In the spirit of the regulirized covariance of \emph{Hotelling.shrink}, we try an $l_2$ regulirized svm \cite{friedman_regularization_2010}, and shrinkage based LDA \citep{pang_shrinkage-based_2009,ramey_high-dimensional_2016}. %TODO: verify references
In the spirit of the diagonilized covariance of \emph{sd}, we try a diagonlized LDA \citep{dudoit_comparison_2002}, which can be thought of a method intersecting Fisher's LDA and Naive Bayes. 


Simulation results reported in Figure~\ref{fig:highdim} with naming conventions in Table~\ref{tab:collected_3}.
It can be seen that regulirizing a classifier in high dimension, just like a parameter test, improves power. 
It can also be seen that (regulirized) parameter tests are still more powerful than (regulirized) accuracy tests. 
This was to be expecetd, since we already saw in (e.g. Figure~\ref{fig:simulation_11}) that the unregulirized parameter test, \emph{Hotelling}, is slightly more powerful than the unregulirized accuracy test, \emph{svm.CV.1} for instance.
...
\hline
\hline
svm.highdim.1 & SVM & TRUE & accuracy & cost=1e-1 \\ 
lda.highdim.1 & LDA & TRUE & accuracy & -- \\ 
lda.highdim.2 & LDA & TRUE & accuracy & -- \\ 
lda.highdim.3 & LDA & TRUE & accuracy & -- \\ 
\end{tabular} 
\captionsetup{type=table}
\caption{
The same as Table~\ref{tab:collected} for regulirized (high dimensional) predictors. 
\emph{svm.highdim.1} is an $l2$ regulirized SVM \cite{friedman_regularization_2010}. 
...
\centering
	  ...
	  \caption{\textbf{HighDim Classifier: TODO}. } %TODO: caption
	\label{fig:highdim}
\end{figure}
