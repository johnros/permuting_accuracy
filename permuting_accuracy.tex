...
This is the \emph{resubstitution classification} in \cite{ramdas_classification_2016}, and simply means that the accuracy is not cross validated. 
If the data is high dimensional, the train accuracy will be very high due to over fitting. 
In an extreme case, the train accuracy will be $1$ for the observed data, but also for any permutation 
...
\item Not surprisingly, if the two groups differ in scale, and not in location, then none of the tests is appropriate (Figure~\ref{fig:scale_change}). 
...
\item Cross-validate. For moderate sample sizes, the power loss due to the holdout inefficiency is smaller than the power loss due to the concentration of the train accuracy. 
...
%	  \includegraphics[width=1\linewidth]{"art/2016-08-05 06:39:27"}
