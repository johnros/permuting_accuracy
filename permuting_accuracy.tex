...
\newcommand{\hypEstim}{\hyp} %{\hat{\hyp}} % A hypothesis
...
An accuracy test amounts to learning a predictive model $\hyp(\features)$ from some assumed model class $\hyp \in \hypclass$. 
The prediction accuracy, denoted $\acc_{\hyp}$, is defined as the probability of a given classifier $\hyp(x)$ of making a correct prediction.
Denoting by $I(A)$ the indicator function of the event $A$, we get
\begin{align}
	\acc_{\hyp}:=\expect{I(\hyp(x)=y)}
\end{align}
where averaging is over all possible data points, ($\features,\outcomes$). 
...
Given a predictor $\hypEstim$, a natural test statistic its empirical accuracy 
\begin{align}
	\accEstim_{\hyp}:= \sum_i I(f(x_i)=y_i).
\end{align}
...
The of tests we will be comparing is collected in Table~\ref{tab:collected}.
\begin{tcolorbox}
\centering
