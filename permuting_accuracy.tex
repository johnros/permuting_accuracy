...
On the other hand, the analysis in \cite{ramdas_classification_2016} is asymptotic. This eschews the discrete nature of the accuracy statistic, which will shown to have  crucial impact. 
...
\item The conservativeness decays as the sample grows (Figure~\ref{fig:large_sample}), supporting the statement that discretization is responsible for power loss. 
...
\item Balancing typically has no effect. 
...
At the risk of abusing notation, classifiers are designed to solve $\argmin{}{FP+FN}$ where $FP$ and $FN$ are the false positive and false negative error rates, or \emph{type I} and \emph{type II} errors, respectively.
One might thus argue that our simulations are unfair since our simulation benchmark is $\argmin{}{FN \; s.t. \; FP \leq \alpha}$, as in the classical Neyman-Pearson framework for which the location tests are designed. 
Our view is that this is not the case because, and the comparison is indeed ``fair''. 
This is because the permutation test ensures that the constraint $FP \leq \alpha$ is satisfied. 



%Neyman-Pearson classifier: \cite{scott_neyman-pearson_2005}
