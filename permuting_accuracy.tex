...
Denoting by 
$i=1,\dots,I$ the subject index, and 
$v=1,\dots,V$ the voxel index.
...
\label{algo:statistic}
 ...
 \KwResult{Brain map of group statistics: $\{\bar{T}_v\}_{v=1}^V$}
	 \For{$v \in 1,\dots,V$}{
		 \For{$i \in 1,\dots,I$}{
			 $T_{i,v} \leftarrow$ observed test statistic for subject $i$ in a region centered at $v$.
			 } 	  
...
\label{algo:permutation}
 ...
 \KwResult{Regions encoding differences between stimuli.}
  \For{$i \in 1,\dots\,m$}{
 	 \For{$v \in \text{brain volume}$}{
 	 
 	 }
  }
  $T_i \leftarrow $ test statistic for subject $i$ \;
 $\bar{T} \leftarrow \frac{1}{m} \sum T_i$  \;
 average test statistic for over subjects\;
 \For{$i \in \text{number of permutation}$}{
	 \For{$v \in \text{brain volume}$}{
