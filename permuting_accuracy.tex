...
This is not the case, however, for the \emph{leave-one-out bootstrap estimator} (B-LOO) \cite[Sec 7.11]{hastie_elements_2003}. 
It is a simplified version of the \emph{0.632 bootstrap estimator} (B-$0.632$)\citep{efron_improvements_1997}, and suffices for our purpose since we are not interested in unbiased risk estimation, but merely signal detection.
%TODO: define the two bootstrap estimators. 
...
The simlation results are reported in Figure~\ref{fig:bootstrap}.
It can be seen that selecting test sets with replacement does increase the power, when compared to V-fold cross validation. 
...
\end{tabular} 
\captionsetup{type=table}
\caption{The same as Table~\ref{tab:collected} for bootstraped accuracy estimates. } %TODO: caption. 
