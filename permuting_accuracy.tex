...
Using the results of \cite{ramdas_classification_2016}, we derive that when signal is manifested in a location shift, the ARE between Hotelling's $T^2$ (location) test and Fisher's LDA (accuracy) test is $\sqrt{2 \pi} \approx 2.5$. 
This means that Fisher's LDA requires $2.5$ more samples to achieve the same (asymptotic) power than the $T^2$ test. 
This means that Fisher's LDA is remarakbly inneficient. 
