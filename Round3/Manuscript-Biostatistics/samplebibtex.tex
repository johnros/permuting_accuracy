...
Two-groups hypothesis tests may be divided into tests for equality of location (i.e., equality of means), and two-sample goodness of fit tests (i.e., equality of the two whole distribution, GOF in short).
...
Crucially for our applications, we will assume that the number of samples is in the order of the dimension of each sample, or smaller. 
...
Instead, we merely seek to demonstrate that accuracy tests are never optimal, compared to bona-fide two-group tests. 

Various authors have compared accuracy tests to two-group tests, often with contradicting conclusions.
\cite{yu2007two}, for instance, find that an accuracy test based on a tree predictor is preferable over a two-group test. 
...
\cite{lopez2016revisiting} compare several accuracy tests to several location and GOF tests and conclude that a neural-net based accuracy test is preferable. 
...
\begin{figure}[h]
	...
	%		\centering
	%		\includegraphics[width=1\columnwidth]{"art/file3"}
	...
	\begin{subfigure}{.7\columnwidth}
		...
		%		\caption{\textbf{Balanced 4-fold CV.}} 
		\label{fig:file2}
	\end{subfigure}
	\caption{
