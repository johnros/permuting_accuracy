...
p <- 230
n <- 400 # total sample size-> n/2 in each group.
n.folds <- 4

source('Gilron Effect of Cross Validatino.R')

# effects <- seq(0, 10, length=3) this signal is too weak
effects <- seq(0, 20, length=3) # 
...
SNR:
$$\frac{n}{2} \mu'\Sigma^{-1}\mu=\frac{n}{2} c^2 e'\Sigma^{-1}e$$ where $e$ is a direction vector.
...
pvals.1.9 <- foreach(j=1:n.replications, .combine=rbind) %dopar%{
  foreach(effect=effects, .combine=rbind)  %do% {
    ...
    # Observables 
    ts <-statisticsLargeSample(x1 = x1, 
                    x2 = x2, 
                    Sigma.inv = Sigma.inv, 
                    noise = noise, 
                    labels = labels, 
    ...
    for(i in 1:n.permutations){
      ...
      permuted[i,] <- statisticsLargeSample(x1 = x1, 
                                 x2 = x2,
                                 Sigma.inv =Sigma.inv, 
                                 noise = noise,
                                 labels = labels,
