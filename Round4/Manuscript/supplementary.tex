...
The larger the main effects, the larger the different in location.
...
It may also be surprising that in the original space, $x$, it is easier to detect main effects only, than main effects with interactions (Fig.\ref{fig:interactions-in-original-space} vs. Fig.\ref{fig:main-only-original-space}). 
Why does adding more signal (i.e. effects) reduces power? 
Well, the signal added is not part of the $x$ space, so for detection algorithm operating in $x$ (not $\tilde{x}$), this is actually part of the noise. 
...
\begin{figure}[th]
	...
	\begin{subfigure}[t]{.45\columnwidth}
		...
		\caption{TODO: update. Data analyzed in augmented interactions space ($\tilde x$).}
