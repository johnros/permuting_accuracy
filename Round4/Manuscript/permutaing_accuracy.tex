...
\label{sec:logistic}
Shift alternatives are (arguably) the most studied signal model in the statistical literature. 
...
Logistic regression is not a class shift, because when fixing $P(y|x)$, there is no marginal distribution of $x$ for which $x_1 $ is a shifted version of $x_0$.
...
In the SM we separate between main effects and second order interactions, and discover that the shift-like main effects drive the power of these tests. 
