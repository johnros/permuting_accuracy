...
To prove this point, we increase the scale of the problem by one order of magnitude: we fix $p/n$ at $23/40$, and set $n=4,00, p=2,30$. 
The results are qualitatively similar to the high-dim--small-sample of, and reported in the SM.
...
% logistic regression
% mixture class
Shift alternatives are by far the most studied in the statistical literature. 
This is due to mathematical convenience, but also for empirical reasons: 
(a) Many effects are mere shifts after a scale transformation. For instance, a multiplicative effect in log scale, or a second degree interaction in the augmented space. 
(b) Many effects are not ``pure shifts'', but have a shift component. In fact, it would be quite unorthodox to assume an effect is manifested in higher moments alone. 

For completeness, we also report power for logistic regression.
Logistic regression is not a shift, because when fixing $P(Y|x)$, there is no marginal distribution of $X$ for which $X_1 $ is a shifted version of $X_0$.
To further depart from a shift class, we add second order interactions.
In Figure~\ref{fig:interactions} we report the usual power of our tests under these assumptions.
The figure demonstrates that two-group tests still dominate in power, even when the problem gravely departs from the shift class. 

\begin{figure}[th]
	\centering
	\includegraphics[width=0.5\columnwidth]{"art/file42"}
	\caption{\textbf{Interactions In Logistic Regression.} 
		Data generated via $Y|x \sim Binom(1,p(x));p(x)=\exp(\eta)/[1+\exp(\eta)];\eta=x'Bx; X\sim \mathcal{N}(0,I_{p\times p})$.
	Data analyzed in the original $x$ space.  } 
	\label{fig:interactions}
\end{figure}


More variations on the logistic setup can be found in the Supplementary Material (SM). 
In particular: 
(i) Logistic regression with main effects only.
(ii) Logistic regression with second order interactions only. 
...
In the Gaussian shift-class examples, we fix $KL[\bar x_1,\bar x_0]=c^2 p$, which is the same as fixing $\Vert \mu \Vert^2_\Theta$.
...
For verification, in the SM we also report the power when fixing the Euclidean norm between population means. 
...
In the SM we report the power when $\mu$ is sparse. 
...
The logistic regression with second order interactions in the SM is such an example.
...
The web supplementary material, available at [TODO: add url]. 
