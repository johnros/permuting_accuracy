---
title: "Logistic Regression (quadratic form in augmented space)"
...
p.total <- p+1+p.quadratic
beta0 <- c(rep(0,p+1),rep(1,p.quadratic))
...
pvals.1.9 <- foreach(j=1:n.replications, .combine=rbind) %dopar%{
  foreach(effect=effects, .combine=rbind)  %do% {
    ...
    Sigma.inv <- makePrecision(noise.augment)
    ...
    for(i in 1:n.permutations){
      ...
      permuted[i,] <- statisticsNoOracle(x1 = x1, 
                                         x2 = x2,
                                         noise = noise, 
                                         labels = labels, 
                                         fold.ids = fold.ids)
